...
func (insta *Instagram) GetSessions(u *url.URL) []*http.Cookie {
	if u == nil {
		u, _ = url.Parse(GOINSTA_API_URL)
	}
	return insta.Cookiejar.Cookies(u)
}
...
// GetCSRFToken returns back the csrf token saved as cookie
func (insta *Instagram) GetCSRFToken() string {
	for _, value := range insta.GetSessions(nil) {
...
const (
	...
	GOINSTA_CHALLENGE_SMS   ChallengeDelivery = "0"
	GOINSTA_CHALLENGE_EMAIL ChallengeDelivery = "1"
	NoCSRFToken                               = "None"
)
...
func NewViaProxy(username, password, proxy string, optChallenge *Challenge) *Instagram {
	insta := New(username, password, optChallenge)
	insta.Proxy = proxy
	return insta
}
...
// Instagram struct
func New(username, password string, optChallenge *Challenge) *Instagram {
...
func (insta *Instagram) requestChallengeCode() (body []byte, err error) {
	var v = url.Values{
		"choice": {string(insta.challengeOptions.Choice())},
	}
	return insta.sendRequest(&reqOptions{
		Endpoint:    insta.challengePath,
		PostData:    string(v.Encode()),
		IsLoggedIn:  true,
		IsChallenge: true,
	})
}

func (insta *Instagram) submitChallengeCode() (body []byte, err error) {
	if insta.Informations.Token == "" {
		insta.Informations.Token = insta.GetCSRFToken()
	}
	var v = url.Values{
		"security_code":       {insta.challengeOptions.Code},
		"guid":                {insta.Informations.UUID},
		"login_attempt_count": {"0"},
		"_csrftoken":          {insta.Informations.Token},
		"device_id":           {insta.Informations.DeviceID},
